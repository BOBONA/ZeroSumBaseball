class Zone:
    ...
    def __init__(self, coords: list[tuple[int, int]], left: float, right: float, bottom: float, top: float, is_strike: bool = True):
        self.coords = coords
        self.left = left
        self.right = right
        self.bottom = bottom
...
for i in range(len(x_divisions) - 1):
    for j in range(len(y_divisions) - 1):
        ZONES.append(Zone([(i + 1, j + 1)], x_divisions[i], x_divisions[i + 1], y_divisions[j], y_divisions[j + 1]))
